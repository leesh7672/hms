...
def textify(e, spell, ident, coder=tex):
    ...
    if part != None:
        if part != '':
            ...
            beforehand = True
    return total.strip()
categories = {'comp':"首詞", 'infl':"助詞", 'adv':"副詞", 'prep': "介詞", 'verb': "動詞", 'co': "連詞", 'det': "指詞", 'adj': "定詞", 'noun': "名詞", 'cl': "量詞", 'num': "數詞"}
...
def _spell(x, num):
    ...
    for ch in respell:
        ...
        total += 1000000000000000000000000000000000000000000000000000000*(int(kangxi[0]) * 1000 + int(kangxi[1]))/counter
        counter*= 10000000
    return total*1000 - int(num)
def update():
