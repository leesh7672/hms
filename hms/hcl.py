...
def textify(e, spell, ident, coder=tex):
    ...
    if part != None:
        if part != '':
            ...
            beforehand = True
    return total.strip()
categories = {'comp':"成", 'infl':"助", 'adv':"狀", 'lv':"態", 'verb': "謂", 'prep': "縛", 'co': "結", 'det': "指", 'adj': "定", 'noun': "體", 'cl': "量", 'calc': "算", 'num': "數"}
...
def build():
    ...
    for result in results:
        ...
        txt+= "\\entry{{{}}}{{{}}}{{{}{}{}}}{{{}}}".format(spell.repace('(', '\textit{').replace(')', '}'), num, spells, definition_txt, cites, '')
    return txt
def initialize():
    global c
